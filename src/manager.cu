...
GPUTransformer::GPUTransformer (float* point_host_, int size_, int* x_, int* y_, int* height_, int max_length_, int max_height_, int num_x_, int num_y_, int num_height_, int outsize_) {
  ...
  if((outsize / d_grid_size) > 1){
    density = outsize / d_grid_size;
  }else{
    density = 1;
  }
...
void GPUTransformer::transform() {
  ...
  point2gridmap<<<gridSize, blockSize>>>(point_device, x, y, height, d_size, h_max_length, h_max_height, h_num_x, h_num_y, h_num_height);
...
void GPUTransformer::retreive(float* point_transformed) {
  ...
  while(total != outsize)
  {
    ...
    {
      if(counter[y_h[i] + x_h[i] * h_num_y + height_h[i] * h_num_y * h_num_x] <= density && total < outsize)
      {
        total ++;
